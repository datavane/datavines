...
public class ColumnHistogram extends BaseSingleTableColumn {
    ...
    public ExecuteSql getActualValue(Map<String,String> inputParameter) {
        ...
        if (!filters.isEmpty() && srcConnectorType.equals("oracle")) {
            actualValueSql.append(" and ${limit_top_50_key} group by ${column} order by count desc) T");
        }else if(filters.isEmpty() && srcConnectorType.equals("oracle")){
            actualValueSql.append(" where ${limit_top_50_key} group by ${column} order by count desc ) T");
        }else{
