...
public class AuthenticationInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        try {
            ...
            response.setStatus(ApiStatus.REQUEST_ERROR.getCode());
            return false;
        }
        ...
        if (Objects.isNull(token)){
            throw new DataVinesServerException(ApiStatus.TOKEN_IS_NULL_ERROR);
        }
        String username = tokeManager.getUsername(token);
        User user = userService.getByUsername(username);
        if (null == user) {
            throw new DataVinesServerException(ApiStatus.INVALID_TOKEN, token);
        }
